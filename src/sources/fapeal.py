...
class FAPEALSource(BaseSource):
    USER_AGENT = {'User-Agent': 'editais-bot/1.0'}
    INCLUDE_HINTS = ('edital', 'chamada', 'inscri', 'internacional', 'submiss', 'oportunidade')
    ...
    def parse(self, raw_content: str) -> list[dict[str, Any]]:
        ...
        for anchor in soup.select('main a[href], article a[href], .entry-content a[href]'):
    ...
    def _extract_notice_link(self, soup: BeautifulSoup) -> str | None:
        for anchor in soup.select('a[href]'):
            ...
            if lower_href.endswith('.pdf') or '.pdf?' in lower_href:
                return full_href
            if any(token in label for token in ('edital', 'chamada', 'submiss', 'inscri')):
                return full_href
        return None
    ...
    def _infer_categoria(self, title: str, summary: str) -> str:
        ...
        if 'bolsa' in combined:
            return 'bolsa'
        if any(token in combined for token in ('inov', 'empreendedor', 'startup', 'empresa')):
            return 'inovacao'
        return 'pesquisa'

    def _infer_publico_alvo(self, title: str, summary: str) -> str:
        combined = f'{title} {summary}'.lower()
        if 'internacional' in combined:
            return 'Pesquisadores e instituicoes de pesquisa de Alagoas em cooperacao internacional'
        if any(token in combined for token in ('empresa', 'startup', 'inov')):
            return 'Empresas, empreendedores, pesquisadores e ICTs de Alagoas'
        return 'Pesquisadores, grupos de pesquisa e instituicoes cientificas de Alagoas'
