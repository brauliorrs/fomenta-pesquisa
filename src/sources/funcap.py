...
class FUNCAPSource(BaseSource):
    USER_AGENT = {'User-Agent': 'editais-bot/1.0'}
    INCLUDE_HINTS = ('edital', 'inscri', 'lançamento', 'lancamento', 'chamada')
    EXCLUDE_HINTS = (
        'resultado',
        'retifica',
        'retificação',
    ...
    def _build_item(self, fallback_title: str, detail_url: str) -> dict[str, Any] | None:
        ...
        summary = self._extract_summary(soup) if soup is not None else fallback_title
    ...
    def _extract_notice_link(self, soup: BeautifulSoup) -> str | None:
        for anchor in soup.select('a[href]'):
            ...
            if 'montenegro.funcap.ce.gov.br/sugba/edital/' in lower_href:
                return full_href
            if lower_href.endswith('.pdf') or '.pdf?' in lower_href:
                return full_href
            if any(token in label for token in ('edital', 'inscri', 'chamada')):
                return full_href
        return None

    def _extract_opening_date(self, soup: BeautifulSoup) -> str | None:
        for node in soup.select('time, .date, .published'):
    ...
    def _looks_open_or_upcoming(self, title: str, summary: str) -> bool:
        ...
        if any(hint in combined for hint in self.EXCLUDE_HINTS):
            return False
        return any(token in combined for token in self.INCLUDE_HINTS)

    def _infer_categoria(self, title: str, summary: str) -> str:
        combined = f'{title} {summary}'.lower()
        if any(token in combined for token in ('bolsa', 'mestrado', 'doutorado')):
            return 'bolsa'
        if any(token in combined for token in ('empreendedora', 'empreendedor', 'inova', 'impacto', 'startup')):
            return 'inovacao'
        return 'pesquisa'

    def _infer_publico_alvo(self, title: str, summary: str) -> str:
        combined = f'{title} {summary}'.lower()
        if any(token in combined for token in ('empreendedora', 'empreendedor', 'startup', 'empresa', 'impacto')):
            return 'Empreendedores, pesquisadores, startups e ICTs do Ceara'
        if any(token in combined for token in ('mestrado', 'doutorado', 'bolsa')):
            return 'Programas de pos-graduacao, bolsistas e pesquisadores do Ceara'
        return 'Pesquisadores, grupos de pesquisa e instituicoes cientificas do Ceara'
