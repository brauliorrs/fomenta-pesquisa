...
class FAPEPISource(BaseSource):
    ...
    def parse(self, raw_content: str) -> list[dict[str, Any]]:
        ...
        for article in soup.select('article, .post, .elementor-post'):
    ...
    def _looks_open_or_upcoming(self, title: str, summary: str) -> bool:
        ...
        if any(hint in combined for hint in self.EXCLUDE_HINTS):
            return False
        return any(token in combined for token in ('edital', 'chamada', 'inscri', 'submiss', 'oportunidade'))

    def _infer_categoria(self, title: str, summary: str) -> str:
        combined = f'{title} {summary}'.lower()
        if 'bolsa' in combined:
            return 'bolsa'
        if any(token in combined for token in ('inov', 'startup', 'empresa', 'centelha')):
            return 'inovacao'
        return 'pesquisa'

    def _infer_publico_alvo(self, title: str, summary: str) -> str:
        combined = f'{title} {summary}'.lower()
        if any(token in combined for token in ('empresa', 'empreendedor', 'startup', 'inov')):
            return 'Empresas, empreendedores, pesquisadores e ICTs do Piaui'
        if 'bolsa' in combined:
            return 'Estudantes, bolsistas e pesquisadores vinculados a instituicoes do Piaui'
        return 'Pesquisadores, grupos de pesquisa e instituicoes cientificas do Piaui'
